...
Run <- function(s,x,y,p,M = "NICHT_DEF"){
  ...
  for(ii in 1:s){
    ...
    name <- paste("plot", ii, ".png", sep ='' )
    print(name)
    png(name)
    cat(dim(M),"\t",ii, "\t", round(((sum(M)*100)/(dim(M)[1]*dim(M)[2])),digits = 2),"%\n")
    im <-image(M, col = grau,asp=1,x = seq(0,dim(M)[1]-1,length.out = dim(M)[1]),
                             y = seq(0,dim(M)[2]-1,length.out = dim(M)[2]))
    Warten(0.2)
    dev.off()
  }
  
  my_command <- 'magick convert *.png -loop 0 -delay 100 stick_figure.gif'
  system(my_command)
}
